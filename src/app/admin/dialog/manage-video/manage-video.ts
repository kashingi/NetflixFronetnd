...
export class ManageVideo implements OnInit {
  ...
  onVideoPicked(ev: Event) {
    ...
    if (!hasValidMimeType && !hasValidExtension) {
      this.notification.error('Please select a valid file (MP4, MKV etc.)')
    }
  ...
  onSave() {
    ...
    op$.subscribe({
      ...
      error: (err) => {
        this.isSaving = false;
        this.errorHandler.handleError(err, 'Failed to save video, please try again');
      }
    });
  }
